...
object MapProjection {
    ...
    fun tileX(lon: Double, zoom: Int): Int =
        floor(lonToWorldX(lon, zoom) / TILE_PX).toInt().coerceIn(0, (1 shl zoom) - 1)

    /** OSM tile Y index containing [lat] at [zoom]. */
    fun tileY(lat: Double, zoom: Int): Int =
        floor(latToWorldY(lat, zoom) / TILE_PX).toInt().coerceIn(0, (1 shl zoom) - 1)
    ...
    fun fitZoom(
        ...
        canvasW: Float, canvasH: Float,
        minZoom: Int = 5, maxZoom: Int = 18,
    ): Int {
        for (z in maxZoom downTo minZoom) {
            val x1 = lonToWorldX(minLon, z); val x2 = lonToWorldX(maxLon, z)
            val y1 = latToWorldY(maxLat, z); val y2 = latToWorldY(minLat, z)
